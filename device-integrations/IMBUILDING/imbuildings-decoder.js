    ...
    switch(parsedData.payload_variant){
        case 0x01:
            if(input.payloadHeader !== false){
                ...
                parsedData.sdi_device_status = input.bytes[8];
            ...
            break;
        case 0x03:
            parsedData.sdi_device_status = input.bytes[input.bytes.length - 10];
            ...
            break;
        case 0x06:
            parsedData.sdi_device_status = input.bytes[input.bytes.length - 4];
...
function parsePeopleCounter(input, parsedData){
    switch(parsedData.payload_variant){
        case 0x04:
            ...
            break;
        case 0x06:
            parsedData.sdi_device_status = input.bytes[input.bytes.length - 13];
...
function parseButtons(input, parsedData){
    switch(parsedData.payload_variant){
        case 0x03:
            ...
            parsedData.button_pressed = (input.bytes[input.bytes.length - 1] != 0) ? 1 : 0;
            parsedData.button = {
        ...
        case 0x04:
            ...
            case 0x05:
                if(input.payloadHeader !== false){
                    parsedData.sdi_device_status = input.bytes[input.bytes.length - 6];
                ...
                parsedData.button_pressed = (input.bytes[input.bytes.length - 3] != 0) ? 1 : 0;
                parsedData.button = {
            ...
            case 0x06:
                if(input.payloadHeader !== false){
                    parsedData.sdi_device_status = input.bytes[input.bytes.length - 15];
...
function decodeUplink(hexString) {
    let input = {bytes: Buffer.from(hexString, 'hex')};
    var d = Object.assign({received_at: new Date().toISOString()}, _decodeUplink(input).data);

    if (d.button ) {
        d.button_1 = (d.button.a) ? 1 : 0;
        d.button_2 = (d.button.b) ? 1 : 0;
        d.button_3 = (d.button.c) ? 1 : 0;
        d.button_4 = (d.button.d) ? 1 : 0;
        d.button_5 = (d.button.e) ? 1 : 0;
    }
